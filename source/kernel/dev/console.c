...
int console_init (int idx) {
    ...
    if (idx == 0) {
        int cursor_pos = read_cursor_pos();//不直接覆盖原来的输出内容，而是去读取光标位置
...
static void clear_esc_param (console_t * console) {
	memset(console->esc_param, 0, sizeof(console->esc_param));
 ...
 * 可能有多个进程在写，注意保护
 */
int console_write (int idx,const char* buffer,int len) {
	console_t * console = console_buf + idx;
