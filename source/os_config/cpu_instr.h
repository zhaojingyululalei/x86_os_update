...
static inline void sgdt(uint32_t *base, uint16_t *limit) {
    struct {
        uint16_t limit;
        uint32_t base;
    } __attribute__((packed)) gdt;

    // 使用 sgdt 指令将 GDTR 的内容存储到 gdt 结构体中
    __asm__ __volatile__("sgdt %[g]" : [g]"=m"(gdt));

    // 将限长和基地址存储到传入的指针中
    if (limit) *limit = gdt.limit;
    if (base) *base = gdt.base;
}
